...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('budget_id', 'uuid', (col) =>
      col.references('budgets.id').onDelete('set null'),
    )
    .execute()

  // Backfill: link existing budget categories
  await sql`UPDATE categories SET budget_id = budgets.id FROM budgets WHERE budgets.category_id = categories.id`.execute(db)

  // Drop the unique constraint on (category_id, year) since each budget now creates its own category
  await db.schema
    .alterTable('budgets')
    .dropConstraint('uq_budgets_category_year')
    .execute()
}
