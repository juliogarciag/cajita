...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  for (const table of ['categories', 'movements', 'budgets', 'snapshots', 'checkpoints']) {
    await db.schema
      .alterTable(table)
  ...
  for (const table of ['categories', 'movements', 'budgets', 'snapshots', 'checkpoints']) {
    await db.schema.createIndex(`idx_${table}_team_id`).on(table).column('team_id').execute()
  }
}

export async function down(db: Kysely<unknown>): Promise<void> {
  for (const table of ['categories', 'movements', 'budgets', 'snapshots', 'checkpoints']) {
    await db.schema.dropIndex(`idx_${table}_team_id`).ifExists().execute()
